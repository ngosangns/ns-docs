...
> **Xem thêm:** [[algorithms-data-structures-cheatsheet|Algorithms & Data Structures CheatSheet]]
...
export default function linear_search(haystack: number[], needle: number): boolean {
  for (let i = 0, il = haystack.length; i < il; i++) if (haystack[i] === needle) return true
...
export default function two_crystal_balls(breaks: boolean[]): number {
  ...
  for (let ball2 = ball1 - jumps; ball2 < ball1; ball2++) if (breaks[ball2]) return ball2
